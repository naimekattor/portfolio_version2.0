      ...
      <div className="container mx-auto px-6">
        <motion.div className="flex justify-between items-end mb-16">
          <div>
            ...
            <p className="text-slate-600 max-w-2xl">Deep dives into how I deliver value through technology.</p>
          </div>
          <button className="hidden md:block text-primary-600 font-semibold hover:underline">View all projects</button>
        </motion.div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.tech.map((t, j) => (
                  <span key={j} className="px-2 py-1 bg-white border border-slate-200 rounded text-[10px] font-bold text-slate-500 uppercase tracking-wider">{t}</span>
                ))}
              </div>
              <h3 className="text-2xl font-bold text-slate-900 mb-3">{project.title}</h3>
              <p className="text-slate-600 mb-6 line-clamp-2">{project.solution}</p>
              <div className="p-4 bg-secondary-50 rounded-xl mb-6">
                <p className="text-xs font-bold text-secondary-600 uppercase tracking-wider mb-1">Impact</p>
