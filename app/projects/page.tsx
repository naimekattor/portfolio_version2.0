...
export default function ProjectsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col transition-colors duration-300">
      ...
      {/* Filter Bar */}
      <section className="py-8 bg-slate-100/70 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 sticky top-[73px] z-30 backdrop-blur-md">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:border-primary-600 transition-colors shadow-2xs"
            />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          {filteredProjects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProjects.map((project, idx) => (
                <motion.div
                  ...
                  className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden group"
                >
                  <div>
                    {/* Image / Header Banner */}
                    <div className="h-48 w-full bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                      {project.images && project.images[0] ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-50 to-slate-200 dark:from-slate-800 dark:to-slate-900 text-primary-600 font-bold text-lg">
                          {project.title}
                        </div>
                      )}
                      {project.impact && (
                    ...
                    <div className="p-6">
                      ...
                      <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2.5 group-hover:text-primary-600 transition-colors">
                        {project.title}
                      </h3>
                      ...
                      {project.technologies && project.technologies.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-4">
                          {project.technologies.map((tech: string, tIdx: number) => (
                            <span
                              key={tIdx}
                              className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[11px] font-medium rounded-md"
                            >
                              {tech}
                            </span>
                          ))}
                  ...
                  <div className="p-6 pt-0 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between mt-4">
                    {project.liveUrl ? (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-primary-600 dark:text-primary-400 hover:text-primary-800 transition-colors"
                      >
            ...
            <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">{t('projectsPage.noProjectsTitle')}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">{t('projectsPage.noProjectsSub')}</p>
            </div>
          )}
        </div>
      </section>
